...
public class Solution {
    ...
    public boolean isSuccessful(GameModel model)
    {
        ...
        for(int i = 0; i < height; ++i)
        {
            for(int j = 0; j < width; ++j)
            {
                ...
                else if(!model.isON(i,j) && !oddNeighborhood(i,j))
                {
                	return false;
                }
            }
    ...
    */
    ...
    public boolean stillPossible(boolean nextValue, GameModel model)
    {
        ...
        int i = currentIndex/width;
        int j = currentIndex%width;
        boolean stateOfCenter = model.isON(i, j);
        boolean before = board[i][j];
        board[i][j] = nextValue;
        // upper validation
        if(i > 0) 
        {
        	boolean stateOfUpper = model.isON(i-1, j);
        	if(stateOfUpper && oddNeighborhood(i-1, j))
        	{
        		possible = false;
        	}
        	else if(!stateOfUpper && !oddNeighborhood(i-1, j))
        	{
        		possible = false;
        	}
        }
        // last line validation
        if(i == (height - 1) && possible)
        {
        	if(j > 0)
        	{
        		boolean stateOfLeft = model.isON(i, j-1);
        		if(stateOfLeft && oddNeighborhood(i, j-1))
        		{
        			possible = false;
        		}
        		else if(!stateOfLeft && !oddNeighborhood(i, j-1))
        		{
        			possible = false;
        		}
        		if(j == (width-1) && possible)
        		{
        			if(stateOfCenter && oddNeighborhood(i,j))
        			{
        				possible = false;
        			}
        			else if(!stateOfCenter && !oddNeighborhood(i, j))
        			{
        				possible = false;
        			}
        		}
...
*/
    ...
    public boolean finish(GameModel model){
        ...
        while(currentIndex < height*width) {
            if(stillPossible(true, model) == true)
            {
                setNext(true);
            }
            else if(stillPossible(false, model) == true)
            {
                setNext(false);
            }
            else 
